...
7. **outbox** (client-side) — the offline write exception (#72). **v1 (issue 11):** attendance ticks + meeting creation queue on-device and upload on reconnect, replayed in order, server assigns timestamps; calendar reads from cache offline (#61 as narrowed by #70). **Extended (issue 18, #72 amended 2026-09-02):** person + group creation join the queue, so replay becomes a dependency graph over client-temp ids, and the People/Groups screens carry pending/failed row states. Settings and Reports stay online-only and say so. Still an append-and-send queue, not a merge engine; single writer (#1); the word is **Upload** (#66/#72). Interface: `enqueue()`, `flush()`, pending count for the UI.
...
- **Local-first + sync engine** (#70 reversed #12/#14) — retired unbuilt; the outbox is the only offline write path. It covers attendance ticks, meeting creation, and — per #72 as amended 2026-09-02 — person and group creation. Editing/archiving, settings and reports stay online-only.
