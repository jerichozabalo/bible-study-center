...
- **2026-09-03 — Issue 16: a stale group-edit form can re-adopt a retired book.**
  ...
  bookmarked/stale `/people/groups/[id]/edit` form or a hand-crafted POST could
  set a retired custom book as a BGroup's `current_book_id`. Issue 16 filters the
  two *list* functions and deliberately leaves `getBook` returning retired books
  by id, so the group still renders fine — the only loss is that the picker's
  "retired" promise isn't enforced server-side on write. Reachable only by the
  signed-in leader with a stale tab. Fix alongside the next `groups.ts` change.
